...
d = {n: False for n in range(1, 10001)}
lychrelNumber = 0

for key, _ in list(sorted(d.items())):
    keyB = key
    r = key + _rN(key)

    counter = 49
    while not isPalindrome(r) and counter > 0:
        key = r
        r = key + _rN(key)
        #print("   ", key, _rN(key), r, isPalindrome(r), "counter: ", counter)
        counter -= 1
    else:
        if isPalindrome(r):
            pass
        elif not isPalindrome(r):
            lychrelNumber += 1
            #print("Lychrel-Number: ", keyB)
        else:
            print("NEVERMIND!")

print("Lychrel-Numbers total: ", lychrelNumber)
